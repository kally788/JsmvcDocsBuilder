 ...
 * @exte $jsmvc$.core.FacadeAbs
 * @example
 * 导入：在 index.html 文件的 head 标签中导入 run 脚本文件。
 * <code>
 * &lt;script type="text/javascript" src="$jsmvc$/run.js" charset="utf-8"&gt;&lt;/script&gt;
 * </code>
 * 加载资源：定义您项目要加载的 CSS,HTML,JS 资源文件。
 * 在 index.html 文件的  <head> 标签中创建一个新的  <script> 标签，并在标签中定义要加载的资源文件数组。
 * <code>
 * //您项目的CSS文件列表
 * var css = [
 *     "css/main.css"
 * ];
 *
 * //您项目的JS文件列表，注意JS数组是个二维数组，用于区分需要同步加载的JS
 * //加载顺序参考：[[a,b,c],[d,e,f],[g,h,i]] 框架会并行加载a、b、c文件，然后再加载 d、e、f，然后在加载 g、h、i ，依此类推
 * //在同一纬度中的JS文件加载顺序是不固定的，因为是并行加载。如果您的JS文件存在依赖关系时，需要拆分成同步方式来加载
 * //例如 b 脚本必须在 a 脚本加载完成后才能执行时（b脚本调用到a脚本中的方法或属性）。那么需要这样定义 [[a],[b]]，而不要这样定义 [[a,b]]。
 * //对于无依赖关系的脚本文件建议采用并行的方式加载，这样能提高加载速度。
 * //通常情况下只有 library 相关文件才可能存在依赖，对于项目的模块不会存在依赖问题，因为模块文件是在全部加载完毕后才被执行的。
 * var js = [[
 *       //加载框架超类
...
window["$jsmvc$"] = {
    run:new function () {
         ...
         * @type function
         * @name start
         * @desc 加载开始，该方法只允许被调用一次，最佳调用方式应放在 body 标签的 onLoad 中
         * @param css:array CSS文件URL列表，并行加载
         * @param js:array JS文件URL列表（二维数组），同一维中的文件并行加载，否则为同步加载
         * @param html:array HTML文件URL列表，并行加载
         * @param ext:array 预加载扩展模块列表，并行加载
         * @param vers:string 资源版本号
         * @example 在 <body> 标签的 onload 事件中添加 start 方法
         * <code>
         ...
         * @type function
         * @name includeCSS
         * @desc 加载CSS文件。CSS文件不能是空内容，否则当失败处理。已经加载过的直接返回成功
         ...
         * @type function
         * @name includeHTML
         * @desc 加载HTML文件，已经加载过的会直接返回上次加载的缓存
        ...
        /**
         * 当框架加载某个资源失败时触发该事件，产生该事件后程序终止
         * @param url    加载失败的资源URL地址
         *
         <example>
        ...
        /**
         * 当框架加载资源超时触发该事件，该事件只会产生一次。超时时间可以通过 setLoadTimeout 设置
         * @param urlObject    加载超时后仍未完成加载的资源URL列表
         *
         <example>
        ...
        /**
         * 当框架不兼容浏览器时触发该事件，产生该事件后程序终止。兼容浏览器可以通过 setIEVerLimit 设置
         *
         <example>
        ...
        /**
         * 限制低的IE版本。最低为6，设置低于6时默认为6，默认8
         *
         <example>
            $jsmvc$.run.setIEVerLimit = 6;
         </example>
         */
