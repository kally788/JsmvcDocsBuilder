...
$jsmvc$.core.FacadeAbs=function(template){var model={},page={},contro={};var observer={};var noticeID=0;var errMsg={"err1":"Can't find JS class # ","err2":"Can't find HTML template # ","err3":"Error:Loop extends # "}
...
var findClass=function(classRoute){var classFunc;if(classRoute&&typeof classRoute=="string"){var routeList=classRoute.split(".");var classFunc=window[routeList[0]];if(classFunc){for(var i=1;i<routeList.length;i++){classFunc=classFunc[routeList[i]];if(!classFunc){break;}}}}
if(!classFunc||typeof classFunc!="function"){alert(errMsg.err1+classRoute);throw errMsg.err1+classRoute;}
return classFunc;}
...
return false;}
var sendBroadcast=function(evtName,data){if(!observer[evtName]){return-1;}
...
return obj;}
$jsmvc$.core.PageAbs.prototype.getTemplate=getTemplate;$jsmvc$.core.PageAbs.prototype.attachNotice=attachNotice;$jsmvc$.core.PageAbs.prototype.removeNotice=removeNotice;this.supers={reqModel:reqModel,reqPage:reqPage,reqContro:reqContro,delModel:delModel,delPage:delPage,delContro:delContro,newInstance:newInstance,sendBroadcast:sendBroadcast}
this.reqModel=reqModel;this.reqPage=reqPage;this.reqContro=reqContro;this.delModel=delModel;this.delPage=delPage;this.delContro=delContro;this.newInstance=newInstance;this.sendBroadcast=sendBroadcast;}
$jsmvc$.core.ModelAbs=function(className){this.className=className;var self=this;var data;var onDestroy=function(){self.onDestroy=undefined;$jsmvc$.facade.delPage(className);}
var getData=function(){return data;}
var setData=function(v){data=v;}
this.supers={onDestroy:onDestroy};this.onDestroy=onDestroy;this.onCreate=function(collection){return{getData:getData,setData:setData};};}
$jsmvc$.core.PageAbs=function(className){this.className=className;var self=this;var errMsg={"err1":"Stage must be '<HTML Element> or <JQuery Object>'","err2":"Display must be '<HTML Element> or <JQuery Object> or <HTML Template Route String>'"};var stage;var display;var isJQuery=function(element){return window.jQuery&&element instanceof window.jQuery;}
...
return false;}
var showPage=function(){if(!stage||!display||isActive(true)){return false;}
...
var onDestroy=function(){self.onDestroy=undefined;$jsmvc$.facade.delPage(className);delDisplay();}
...
var delDisplay=function(){if(display){isActive(false)&&isJQuery(display)?display.remove():display.parentNode.removeChild(display);display=undefined;return true;}
return false}
var getTemplate=this.getTemplate;delete this.getTemplate;var attachNotice=this.attachNotice;delete this.attachNotice;var removeNotice=this.removeNotice;delete this.removeNotice;this.supers={showPage:showPage,isActive:isActive,onDestroy:onDestroy};this.isActive=isActive;this.onDestroy=onDestroy;this.showPage=showPage;this.onCreate=function(){return{setStage:setStage,getStage:getStage,setDisplay:setDisplay,getDisplay:getDisplay,delDisplay:delDisplay,getTemplate:getTemplate,attachNotice:attachNotice,removeNotice:removeNotice};};}